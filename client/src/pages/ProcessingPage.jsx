...
const ProcessingPage = () => {
    ...
    const [productsInStock, setProductsInStock] = useState([]); // Danh sách sản phẩm hạt nhân đầu vào lấy từ Kho
    const [targetProducts, setTargetProducts] = useState([]);   // Danh sách thành phẩm đích
    ...
    // State cho Form khởi tạo lệnh
    const [batchForm, setBatchForm] = useState({
        source: '', // Sẽ được tự động điền khi danh sách tải xong
        target: '', // Sẽ được tự động điền khi danh sách tải xong
        weight: '60.0',
        expectedLoss: '12.5'
    });
    ...
    // 2. Tải danh sách Sản phẩm Đầu vào & Đầu ra thực tế từ hệ thống kho
    const fetchDropdownData = async () => {
        try {
            const token = localStorage.getItem('token');
            const config = token ? { headers: { Authorization: `Bearer ${token}` } } : {};

            // Gọi API lấy danh sách tồn kho thực tế (được nạp từ chức năng Nhập kho của bạn)
            const resStock = await axios.get('http://localhost:5000/api/categories/supplier-stock', config);

            let stockData = [];
            if (resStock.data && resStock.data.success && Array.isArray(resStock.data.data)) {
                stockData = resStock.data.data;
            } else if (Array.isArray(resStock.data)) {
                stockData = resStock.data;
            }

            // Nếu DB có dữ liệu kho thực tế thì gán, nếu không sẽ dùng data dự phòng tránh trắng màn hình
            if (stockData.length > 0) {
                setProductsInStock(stockData);
            } else {
                setProductsInStock([
                    { _id: 'in1', name: 'Cà phê nhân xanh - Arabica Cầu Đất' },
                    { _id: 'in2', name: 'Cà phê nhân xanh - Robusta Đắk Lắk' },
                    { _id: 'in3', name: 'Cà phê Culi nhân xanh thượng hạng' }
                ]);
            }

            // Mock hoặc gọi API danh mục thành phẩm đầu ra (Ví dụ các loại túi hạt thành phẩm/quy cách rang)
            setTargetProducts([
                { _id: 'out1', name: 'Hạt thành phẩm - Rang nhạt (Light Roast)' },
                { _id: 'out2', name: 'Hạt thành phẩm - Rang vừa (Medium Roast)' },
                { _id: 'out3', name: 'Hạt thành phẩm - Rang đậm (Dark Roast)' },
                { _id: 'out4', name: 'Cà phê bột xay nguyên chất túi 500g' }
            ]);

        } catch (err) {
            console.error("Lỗi đồng bộ danh mục sản phẩm kho:", err);
            // Setup data dự phòng an toàn khi mất kết nối mạng hoặc API lỗi
            setProductsInStock([
                { _id: 'in1', name: 'Cà phê nhân xanh - Arabica Cầu Đất (Dự phòng)' },
                { _id: 'in2', name: 'Cà phê nhân xanh - Robusta Đắk Lắk (Dự phòng)' }
            ]);
            setTargetProducts([
                { _id: 'out1', name: 'Rang nhạt (Light Roast)' },
                { _id: 'out2', name: 'Rang vừa (Medium Roast)' },
                { _id: 'out3', name: 'Rang đậm (Dark Roast)' }
            ]);
        }
    };

    // Tự động gán giá trị mặc định cho ô Select sau khi data được tải xong
    useEffect(() => {
        if (productsInStock.length > 0 || targetProducts.length > 0) {
            setBatchForm(prev => ({
                ...prev,
                source: prev.source || productsInStock[0]?.name || '',
                target: prev.target || targetProducts[0]?.name || ''
            }));
        }
    }, [productsInStock, targetProducts]);

    // 3. Lấy lịch sử mẻ rang từ API
    const fetchHistory = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/processing/history');
            if (res.data && res.data.success) setHistory(res.data.data);
        } catch (err) {
    ...
    // 4. Xử lý khi nhấn nút Thực thi
    const handleExecute = async () => {
        if (!batchForm.source || !batchForm.target) {
            alert("Vui lòng chọn đầy đủ Nguyên liệu đầu vào và Sản phẩm đích!");
            return;
        }
        ...
        try {
            ...
            if (res.data && res.data.success) {
                alert("Đã khởi tạo lệnh chế biến thành công! Số lượng tồn kho nguyên liệu đã được cập nhật ngầm.");
                fetchHistory();
            }
        } catch (err) {
            alert("Lỗi thực thi lệnh chế biến: " + (err.response?.data?.message || "Lỗi kết nối Server"));
        } finally {
            setLoading(false);
        }
    };
                ...
                <div style={styles.contentPadding}>
                    ...
                    <div style={styles.dashboardGrid}>
                        ...
                        <div style={styles.leftCol}>
                            <div style={styles.formCard}>
                                <div style={styles.formHeader}>
                                    <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '800' }}>Khởi tạo Lệnh Chế biến</h3>
                                    <span style={styles.batchTag}>MẺ RANG REALTIME</span>
                                </div>

                                <div style={styles.inputGrid}>
                                    <div style={styles.inputGroup}>
                                        <label style={styles.label}>NGUỒN ĐẦU VÀO (TỪ KHO NHẬP)</label>
                                        <select
                                            style={styles.select}
                                        ...
                                        >
                                            {productsInStock.map((prod) => (
                                                <option key={prod._id} value={prod.name}>
                                                    {prod.name} {prod.stock ? `(Tồn: ${prod.stock}kg)` : ''}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                    <div style={styles.inputGroup}>
                                        <label style={styles.label}>SẢN PHẨM ĐÍCH (THÀNH PHẨM)</label>
                                        <select
                                            style={styles.select}
                                            value={batchForm.target}
                                            onChange={(e) => setBatchForm({ ...batchForm, target: e.target.value })}
                                        >
                                            {targetProducts.map((targ) => (
                                                <option key={targ._id} value={targ.name}>
                                                    {targ.name}
                                                </option>
                                            ))}
                                        </select>
                                    ...
                                    <div style={styles.inputGroup}>
                                        <label style={styles.label}>KHỐI LƯỢNG RANG (KG)</label>
                                        <input
                                            type="number"
                        ...
                        <div style={styles.historyCard}>
                            <h3 style={{ fontSize: '18px', margin: '0 0 5px 0', fontWeight: '800' }}>Lịch sử Gần đây</h3>
