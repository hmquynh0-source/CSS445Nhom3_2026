...
/**
 * @API_4A: [BƯỚC 1] NHÀ CUNG CẤP PHÊ DUYỆT ĐƠN HÀNG (Dành cho Supplier Portal)
 * Endpoint: POST /api/transactions/:id/approve
 */
app.post('/api/transactions/:id/approve', protect, async (req, res) => {
    try {
        const orderId = req.params.id;
        const transaction = await Transaction.findById(orderId);

        if (!transaction) {
            return res.status(404).json({ success: false, message: "Không tìm thấy lệnh yêu cầu nhập kho này." });
        }
        ...
        // Tự động bấu víu tìm kiếm tên sản phẩm từ bảng Product nếu trường dữ liệu bị bỏ trống
        let targetProductName = transaction.productName;
        if (!targetProductName && transaction.product) {
            const linkedProduct = await Product.findById(transaction.product);
            targetProductName = linkedProduct ? linkedProduct.name : null;
        }

        if (!targetProductName) {
            return res.status(400).json({ success: false, message: "Hệ thống chưa đồng bộ được tên loại hạt." });
        }

        // Cập nhật trạng thái trung gian chờ Admin tiếp quản xe hàng thực tế
        transaction.status = 'SUPPLIER_CONFIRMED';
        transaction.productName = targetProductName;
        await transaction.save();

        io.emit('update_order_status', transaction);

        return res.json({
            success: true,
            message: `Nhà cung cấp xác nhận xuất kho đơn hàng ${transaction.requestId} thành công! Chờ thủ kho tiếp nhận thực tế tại nhà máy.`,
            data: transaction
        });
    } catch (err) {
        console.error("Lỗi duyệt của NCC:", err);
        return res.status(500).json({ success: false, message: "Lỗi xử lý hệ thống phía NCC: " + err.message });
    }
});
